...
const handleRealScan = async (scannedText) => {
  ...
  // Shared viewfinder with medicine image
  const StaticViewfinder = ({ height }) => (
    <div style={{ position: 'relative', width: '100%', height: height || '100%', borderRadius: '12px', overflow: 'hidden' }}>
      ...
      {/* Inner brackets */}
      {[
        { top: '25%', left: '25%', borderTop: '1px solid rgba(0,219,233,0.2)', borderLeft: '1px solid rgba(0,219,233,0.2)' },
        { top: '25%', right: '25%', borderTop: '1px solid rgba(0,219,233,0.2)', borderRight: '1px solid rgba(0,219,233,0.2)' },
        { bottom: '25%', left: '25%', borderBottom: '1px solid rgba(0,219,233,0.2)', borderLeft: '1px solid rgba(0,219,233,0.2)' },
        { bottom: '25%', right: '25%', borderBottom: '1px solid rgba(0,219,233,0.2)', borderRight: '1px solid rgba(0,219,233,0.2)' },
      ...
      {scanStatus === 'found' && (
        <div style={{ position: 'absolute', inset: 0, background: 'rgba(0,245,160,0.1)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 10 }}>
          <div style={{ textAlign: 'center' }}>
            ...
            <div style={{ fontFamily: 'Space Grotesk, sans-serif', fontWeight: 700, fontSize: '16px', color: '#00f5a0', marginTop: '8px' }}>QR Code Mil Gaya!</div>
          </div>
        </div>
      )}
      {/* Progress bar */}
      ...
      <div style={{ position: 'absolute', top: '14px', left: '50%', transform: 'translateX(-50%)', background: 'rgba(10,11,16,0.8)', backdropFilter: 'blur(10px)', border: '1px solid rgba(0,219,233,0.2)', borderRadius: '999px', padding: '5px 14px', whiteSpace: 'nowrap', zIndex: 8 }}>
        <span style={{ fontFamily: 'Space Grotesk, sans-serif', fontSize: '10px', fontWeight: 600, color: '#00dbe9', letterSpacing: '0.1em' }}>
          {scanStatus === 'scanning' ? `Verify ho raha hai... ${scanProgress}%` : scanStatus === 'found' ? '✓ Mil Gaya!' : '● Camera Active'}
        </span>
      </div>
    </div>
  )
  ...
  if (isMobile) {
    return (
      <div style={{ minHeight: '100vh', background: '#0A0B10', display: 'flex', flexDirection: 'column', fontFamily: 'Inter, sans-serif' }}>
        <nav style={{ position: 'sticky', top: 0, zIndex: 50, display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '14px 20px', background: 'rgba(10,11,16,0.9)', backdropFilter: 'blur(24px)', borderBottom: '1px solid rgba(0,219,233,0.1)' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '10px 20px', maxWidth: '500px', margin: '0 auto', width: '100%' }}>
          <button onClick={() => setFlashOn(!flashOn)} style={{ width: '46px', height: '46px', borderRadius: '50%', background: flashOn ? 'rgba(0,219,233,0.2)' : 'rgba(255,255,255,0.05)', border: `1px solid ${flashOn ? 'rgba(0,219,233,0.5)' : 'rgba(255,255,255,0.1)'}`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '20px', cursor: 'pointer', transition: 'all 0.3s' }}>🔦</button>
        ...
        <div style={{ flex: 1, padding: '0 20px', maxWidth: '500px', margin: '0 auto', width: '100%' }}>
          ...
          {/* Status card */}
          <div style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: '18px', padding: '18px', marginBottom: '12px', display: 'flex', alignItems: 'center', gap: '12px' }}>
            ...
            <div>
              <h3 style={{ fontFamily: 'Space Grotesk, sans-serif', fontSize: '15px', fontWeight: 600, color: '#e3e1e9', marginBottom: '3px' }}>Scanning System Active</h3>
              <p style={{ fontFamily: 'Inter, sans-serif', fontSize: '12px', color: '#849495' }}>Align QR code or barcode within the frame</p>
            </div>
          </div>

          {/* Format pills */}
          <div style={{ display: 'flex', gap: '8px', marginBottom: '12px', flexWrap: 'wrap' }}>
            {[{ icon: '🔍', text: 'QR Code' }, { icon: '▦', text: 'Barcode' }, { icon: '📝', text: 'Batch ID' }].map((p, i) => (
              <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '6px', background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '999px', padding: '6px 14px' }}>
                <span style={{ fontSize: '12px' }}>{p.icon}</span>
                <span style={{ fontFamily: 'Space Grotesk, sans-serif', fontSize: '11px', fontWeight: 600, color: '#849495' }}>{p.text}</span>
              </div>
            ))}
          </div>
          ...
          <button onClick={handleSimulateScan} disabled={scanStatus !== 'active'} style={{ width: '100%', padding: '14px', background: 'transparent', border: '1px solid rgba(0,219,233,0.25)', borderRadius: '12px', color: scanStatus !== 'active' ? '#5a6370' : '#00dbe9', fontFamily: 'Space Grotesk, sans-serif', fontSize: '12px', fontWeight: 700, letterSpacing: '0.1em', cursor: scanStatus !== 'active' ? 'not-allowed' : 'pointer', marginBottom: '100px' }}>
            {scanStatus === 'active' ? '▶ DEMO — Simulate QR Scan' : scanStatus === 'scanning' ? `⏳ Verify Ho Raha Hai ${scanProgress}%` : '✓ Redirecting...'}
          </button>
        </div>
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#0A0B10', fontFamily: 'Inter, sans-serif' }}>
      ...
      <nav style={{ position: 'sticky', top: 0, zIndex: 50, display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '16px 48px', background: 'rgba(10,11,16,0.9)', backdropFilter: 'blur(24px)', borderBottom: '1px solid rgba(0,219,233,0.1)' }}>
        ...
        <div style={{ display: 'flex', gap: '10px' }}>
          <button onClick={() => setFlashOn(!flashOn)} style={{ display: 'flex', alignItems: 'center', gap: '8px', background: flashOn ? 'rgba(0,219,233,0.15)' : 'rgba(255,255,255,0.05)', border: `1px solid ${flashOn ? 'rgba(0,219,233,0.4)' : 'rgba(255,255,255,0.1)'}`, borderRadius: '10px', padding: '10px 18px', color: flashOn ? '#00dbe9' : '#e3e1e9', fontFamily: 'Space Grotesk, sans-serif', fontSize: '12px', fontWeight: 600, cursor: 'pointer' }}>🔦 {flashOn ? 'Flash ON' : 'Flash OFF'}</button>
          <button onClick={() => setShowManual(!showManual)} style={{ display: 'flex', alignItems: 'center', gap: '8px', background: showManual ? 'rgba(0,219,233,0.1)' : 'rgba(255,255,255,0.04)', border: `1px solid ${showManual ? 'rgba(0,219,233,0.4)' : 'rgba(255,255,255,0.1)'}`, borderRadius: '10px', padding: '10px 18px', color: showManual ? '#00dbe9' : '#e3e1e9', fontFamily: 'Space Grotesk, sans-serif', fontSize: '12px', fontWeight: 600, cursor: 'pointer' }}>✏️ Manual Entry</button>
          <button onClick={() => router.push('/wallet')} style={{ background: '#00dbe9', border: 'none', borderRadius: '10px', padding: '10px 20px', color: '#001214', fontFamily: 'Space Grotesk, sans-serif', fontSize: '12px', fontWeight: 700, letterSpacing: '0.08em', cursor: 'pointer', boxShadow: '0 0 20px rgba(0,219,233,0.3)' }}>⬡ CONNECT</button>
        </div>
      </nav>
      ...
      <div style={{ position: 'relative', zIndex: 1, display: 'grid', gridTemplateColumns: '1fr 380px', gap: '32px', padding: '40px 80px', maxWidth: '1400px', margin: '0 auto', alignItems: 'start' }}>
        ...
        <div>
          <div style={{ marginBottom: '20px' }}>
            ...
            <p style={{ fontSize: '14px', color: '#849495' }}>Align QR code or barcode within the frame</p>
          </div>

          {/* Camera toggle */}
          <button onClick={() => setCameraMode(!cameraMode)} style={{ marginBottom: '16px', display: 'flex', alignItems: 'center', gap: '10px', background: cameraMode ? 'rgba(255,77,109,0.08)' : 'rgba(0,219,233,0.08)', border: `1px solid ${cameraMode ? 'rgba(255,77,109,0.3)' : 'rgba(0,219,233,0.3)'}`, borderRadius: '12px', padding: '12px 20px', color: cameraMode ? '#ff4d6d' : '#00dbe9', fontFamily: 'Space Grotesk, sans-serif', fontSize: '13px', fontWeight: 700, cursor: 'pointer', letterSpacing: '0.06em' }}>
            {cameraMode ? '✕ Close Camera' : '📷 Open Camera — Scan QR Code'}
          </button>

          {/* Viewfinder */}
          <div style={{ position: 'relative', width: '100%', paddingBottom: '65%', borderRadius: '20px', overflow: 'hidden' }}>
          ...
          {/* Manual entry */}
          {showManual && (
            <div style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(0,219,233,0.25)', borderRadius: '16px', padding: '20px', marginTop: '16px' }}>
              ...
              <div style={{ display: 'flex', gap: '10px' }}>
                ...
                <button onClick={handleVerify} style={{ padding: '12px 24px', background: '#00dbe9', color: '#001214', fontFamily: 'Space Grotesk, sans-serif', fontWeight: 700, fontSize: '13px', border: 'none', borderRadius: '10px', cursor: 'pointer', boxShadow: '0 0 16px rgba(0,219,233,0.35)' }}>VERIFY KAREIN</button>
              </div>
            </div>
          )}
        </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
          ...
          <div style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: '20px', padding: '20px', backdropFilter: 'blur(16px)' }}>
            ...
            {[{ icon: '🔍', text: 'QR Code', sub: 'Standard & Micro QR' }, { icon: '▦', text: 'Barcode', sub: 'Code 128, EAN-13' }, { icon: '📝', text: 'Manual Entry', sub: 'Batch ID format' }].map((f, i) => (
          ...
          <button onClick={handleSimulateScan} disabled={scanStatus !== 'active'} style={{ width: '100%', padding: '16px', background: scanStatus === 'active' ? 'rgba(0,219,233,0.08)' : 'rgba(255,255,255,0.03)', border: `1px solid ${scanStatus === 'active' ? 'rgba(0,219,233,0.3)' : 'rgba(255,255,255,0.08)'}`, borderRadius: '14px', color: scanStatus !== 'active' ? '#5a6370' : '#00dbe9', fontFamily: 'Space Grotesk, sans-serif', fontSize: '13px', fontWeight: 700, letterSpacing: '0.08em', cursor: scanStatus !== 'active' ? 'not-allowed' : 'pointer' }}>
            {scanStatus === 'active' ? '▶ DEMO — Scan Simulate Karein' : scanStatus === 'scanning' ? `⏳ Verifying... ${scanProgress}%` : '✓ Redirecting...'}
          </button>
        </div>
      </div>
      <style>{`@keyframes pulseDot{0%,100%{transform:scale(1);box-shadow:0 0 8px rgba(0,219,233,.8)}50%{transform:scale(1.5);box-shadow:0 0 20px rgba(0,219,233,1)}}`}</style>
