...
export default function HistoryPage() {
  ...
  const fetchHistory = async () => {
    try {
      const res = await fetch(`${API_URL}/verify/history`)
  ...
  const fetchStats = async () => {
    try {
      const res = await fetch(`${API_URL}/verify/stats`)
      const data = await res.json()
  ...
  const formatTimeShort = date => new Date(date).toLocaleString('en-PK', { day:'2-digit', month:'short', hour:'2-digit', minute:'2-digit' })
  ...
  return (
    <div style={{ minHeight:'100vh', background:'#0A0B10', fontFamily:'Inter, sans-serif' }}>
      ...
      <div style={{ padding: isMobile ? '20px 16px' : '28px 20px', maxWidth:'900px', margin:'0 auto', position:'relative', zIndex:1 }}>

        <div style={{ marginBottom: isMobile ? '20px' : '28px' }}>
          <div style={{ display:'flex', alignItems:'center', gap:'8px', marginBottom:'8px' }}>
            <div style={{ width:'6px', height:'6px', borderRadius:'50%', background:'#00dbe9', boxShadow:'0 0 8px #00dbe9' }} />
            <span style={{ fontFamily:'Space Grotesk, sans-serif', fontSize:'10px', fontWeight:700, color:'#00dbe9', letterSpacing:'0.14em' }}>VERIFICATION HISTORY</span>
          </div>
          <h1 style={{ fontFamily:'Space Grotesk, sans-serif', fontWeight:700, fontSize: isMobile ? '22px' : '28px', color:'#e3e1e9', marginBottom:'6px' }}>Scan History</h1>
          <p style={{ fontSize:'12px', color:'#849495' }}>All medicine verification records — blockchain logged</p>
        </div>

        {/* Stats — 2 columns on mobile, 4 on desktop */}
        <div style={{ display:'grid', gridTemplateColumns: isMobile ? 'repeat(2,1fr)' : 'repeat(4,1fr)', gap: isMobile ? '10px' : '12px', marginBottom: isMobile ? '20px' : '24px' }}>
          {[
            ...
            <div key={i} style={{ background:'linear-gradient(135deg, rgba(255,255,255,0.05), rgba(255,255,255,0.02))', border:`1px solid ${s.color}25`, borderRadius:'14px', padding: isMobile ? '12px' : '16px', backdropFilter:'blur(12px)', position:'relative', overflow:'hidden', minWidth:0 }}>
              ...
              <div style={{ fontSize: isMobile ? '16px' : '20px', marginBottom:'6px' }}>{s.icon}</div>
              <div style={{ fontFamily:'Space Grotesk, sans-serif', fontWeight:800, fontSize: isMobile ? '20px' : '24px', color:s.color, lineHeight:1 }}>{s.val}</div>
              <div style={{ fontSize: isMobile ? '10px' : '11px', color:'#849495', marginTop:'4px' }}>{s.label}</div>
            </div>
          ))}
        </div>
        ...
        {loading ? (
          ...
          <div style={{ padding:'40px 20px', textAlign:'center', background:'rgba(255,255,255,0.03)', borderRadius:'20px', border:'1px solid rgba(255,255,255,0.07)' }}>
            <div style={{ fontSize:'40px', marginBottom:'16px' }}>📋</div>
            <h3 style={{ fontFamily:'Space Grotesk, sans-serif', fontSize:'16px', color:'#e3e1e9', marginBottom:'8px' }}>No History Yet</h3>
            <p style={{ fontSize:'12px', color:'#849495', marginBottom:'20px' }}>Scan a medicine to see verification history</p>
          ...
          <div style={{ display:'flex', flexDirection:'column', gap:'10px' }}>
            {history.map((log) => (
              <div key={log._id} onClick={() => router.push(`/result?batch=${log.batchNumber}`)} style={{ background:'linear-gradient(135deg, rgba(255,255,255,0.04), rgba(255,255,255,0.01))', border:'1px solid rgba(255,255,255,0.07)', borderRadius:'16px', padding:'14px', cursor:'pointer' }}>
                <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:'10px' }}>
                  <div style={{ minWidth:0, flex:1 }}>
                    <div style={{ fontFamily:'Space Grotesk, sans-serif', fontSize:'14px', fontWeight:700, color:'#e3e1e9', marginBottom:'3px', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{log.medicine?.name || 'Unknown'}</div>
                    <div style={{ fontFamily:'monospace', fontSize:'11px', color:'#00dbe9' }}>{log.batchNumber}</div>
                  </div>
                  <div style={{ background:getResultBg(log.result), border:`1px solid ${getResultBorder(log.result)}`, borderRadius:'6px', padding:'4px 9px', flexShrink:0, marginLeft:'8px' }}>
                    <span style={{ fontFamily:'Space Grotesk, sans-serif', fontSize:'9px', fontWeight:700, color:getResultColor(log.result), textTransform:'uppercase', whiteSpace:'nowrap' }}>{log.result}</span>
                  </div>
                </div>
                <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', paddingTop:'10px', borderTop:'1px solid rgba(255,255,255,0.05)' }}>
                  <span style={{ fontSize:'10px', color:'#5a6370' }}>📍 {log.deviceInfo?.location || 'Unknown'}</span>
                  <div style={{ display:'flex', gap:'10px' }}>
                    <span style={{ fontFamily:'Space Grotesk, sans-serif', fontSize:'10px', color:'#00dbe9', fontWeight:600 }}>{log.responseTime}ms</span>
                    <span style={{ fontSize:'10px', color:'#5a6370' }}>{formatTimeShort(log.createdAt)}</span>
                  </div>
                </div>
              </div>
            ))}
        ...
        <div style={{ height: isMobile ? '90px' : '40px' }} />
      </div>
