...
export default function LandingPage() {
  ...
  const features = [
    { icon: '🔐', title: 'Cryptographic Proof', desc: 'SHA-256 hashing creates a tamper-proof fingerprint for every medicine batch.' },
    { icon: '⛓️', title: 'Blockchain Ledger', desc: 'Every verification is recorded on an immutable, transparent ledger.' },
    { icon: '📡', title: 'Offline-First', desc: 'Verify medicines using RSA digital signatures — even without internet.' },
    { icon: '🤖', title: 'AI Risk Engine', desc: 'Real-time anomaly detection flags suspicious scanning patterns instantly.' },
    { icon: '⚠️', title: 'Smart Recalls', desc: 'Privacy-preserving batch recalls protect public safety immediately.' },
    { icon: '🌍', title: 'Global Network', desc: 'Built to scale across borders, languages, and regulatory systems.' },
  ]

  const steps = [
    { num: '01', title: 'Register on Blockchain', desc: 'Manufacturers register each batch — a unique cryptographic hash and QR code are generated automatically.' },
    { num: '02', title: 'Scan Anywhere', desc: 'Consumers scan the QR code with any phone camera — online or completely offline.' },
    { num: '03', title: 'Instant Verification', desc: 'Get a verified result in under a second, with full supply chain history and risk analysis.' },
  ]
    ...
    <div style={{ minHeight: '100vh', background: '#0A0B10', fontFamily: 'Inter, sans-serif', position: 'relative', overflowX: 'hidden' }}>
      ...
      <section style={{ position: 'relative', zIndex: 1, padding: isMobile ? '40px 20px 60px' : '80px 48px 100px', maxWidth: '1300px', margin: '0 auto', display: isMobile ? 'flex' : 'grid', flexDirection: isMobile ? 'column' : undefined, gridTemplateColumns: isMobile ? undefined : '1.1fr 0.9fr', gap: isMobile ? '32px' : '40px', alignItems: 'center' }}>
        <div>
          <div style={{ display: 'inline-flex', alignItems: 'center', gap: '8px', background: 'rgba(0,219,233,0.06)', border: '1px solid rgba(0,219,233,0.2)', borderRadius: '999px', padding: '6px 16px', marginBottom: '24px' }}>
            ...
            <span style={{ fontFamily: 'Space Grotesk, sans-serif', fontSize: '11px', fontWeight: 700, color: '#00dbe9', letterSpacing: '0.14em' }}>BLOCKCHAIN VERIFICATION</span>
          </div>
          ...
          <h1 style={{ fontFamily: 'Space Grotesk, sans-serif', fontWeight: 700, fontSize: isMobile ? '34px' : '54px', color: '#e3e1e9', lineHeight: 1.1, marginBottom: '20px' }}>
            Authenticity<br />You Can <span style={{ color: '#00dbe9', textShadow: '0 0 40px rgba(0,219,233,0.4)' }}>Trust</span>
          </h1>

          <p style={{ fontSize: isMobile ? '14px' : '16px', color: '#849495', lineHeight: 1.8, marginBottom: '32px', maxWidth: '480px' }}>
            Every medicine deserves a verified identity. MediVerify uses blockchain cryptography to instantly confirm authenticity — protecting millions of lives from counterfeit pharmaceuticals.
          </p>
      ...
      <section style={{ position: 'relative', zIndex: 1, padding: isMobile ? '20px 20px 50px' : '20px 48px 80px', maxWidth: '1300px', margin: '0 auto' }}>
        <div style={{ textAlign: 'center', marginBottom: isMobile ? '32px' : '48px' }}>
          <h2 style={{ fontFamily: 'Space Grotesk, sans-serif', fontWeight: 700, fontSize: isMobile ? '24px' : '34px', color: '#e3e1e9', marginBottom: '12px' }}>Built for Trust</h2>
          <p style={{ fontSize: '14px', color: '#849495' }}>Six pillars of protection, working together</p>
        </div>
      ...
      <section style={{ position: 'relative', zIndex: 1, padding: isMobile ? '20px 20px 50px' : '20px 48px 80px', maxWidth: '1100px', margin: '0 auto' }}>
        <div style={{ textAlign: 'center', marginBottom: isMobile ? '32px' : '48px' }}>
          <h2 style={{ fontFamily: 'Space Grotesk, sans-serif', fontWeight: 700, fontSize: isMobile ? '24px' : '34px', color: '#e3e1e9', marginBottom: '12px' }}>How It Works</h2>
          <p style={{ fontSize: '14px', color: '#849495' }}>From factory to your hands — in three simple steps</p>
        </div>
      ...
      <section style={{ position: 'relative', zIndex: 1, padding: isMobile ? '20px 20px 50px' : '20px 48px 80px', maxWidth: '1100px', margin: '0 auto' }}>
        <div style={{ background: 'linear-gradient(135deg, rgba(255,255,255,0.04), rgba(255,255,255,0.01))', border: '1px solid rgba(255,255,255,0.07)', borderRadius: '24px', padding: isMobile ? '24px' : '40px', textAlign: 'center' }}>
          <h2 style={{ fontFamily: 'Space Grotesk, sans-serif', fontWeight: 700, fontSize: isMobile ? '22px' : '28px', color: '#e3e1e9', marginBottom: '8px' }}>Global Verification Network</h2>
          <p style={{ fontSize: '13px', color: '#849495', marginBottom: '24px' }}>Active verification nodes across 47 countries</p>
          ...
          <div style={{ display: 'flex', justifyContent: 'center', gap: isMobile ? '20px' : '40px', marginTop: '24px', flexWrap: 'wrap' }}>
            {[{ val: 47, label: 'Countries' }, { val: 4812, label: 'Active Nodes' }, { val: 99, suf: '%', label: 'Network Health' }].map((s, i) => (
